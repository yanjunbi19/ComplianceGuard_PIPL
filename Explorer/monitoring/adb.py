...
import time
import sys
sys.path.append("/workspace/guitest")
from typing import Optional, Union, List
import config
# 🔹 设备配置（全局）
DEFAULT_ADB_DEVICE =config.ADB_DEVICE
...
class ADB(object):
    def __init__(self, device: str = None, debug: bool = False):
        ...
        # 🔹 修改：如果没有指定设备，使用默认设备
        self._device = device if device else DEFAULT_ADB_DEVICE
        ...
        # 🔹 添加：记录使用的设备
        self.logger.info(f"ADB initialized for device: {self._device}")
    ...
    def target_device(self, new_device: str):
        ...
        self.logger.info(f"ADB target device changed to: {new_device}")
    ...
    def shell(
        self, command: Union[str, List[str]], is_async: bool = False, timeout: Optional[int] = None
    ) -> Optional[str]:
        # 🔹 修改：支持字符串命令（兼容旧代码）
...
# 🔹 添加：便捷函数
def get_default_adb() -> ADB:
    """获取默认ADB实例（使用默认设备）"""
    return ADB(device=DEFAULT_ADB_DEVICE)


def set_default_device(device: str):
    """设置默认设备"""
    global DEFAULT_ADB_DEVICE
    DEFAULT_ADB_DEVICE = device
    logging.info(f"Default ADB device set to: {device}")
